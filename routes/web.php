...
//Webhook for bank transfers
Route::controller(MoneyOutController::class)->prefix('withdraw')->name('money.out.')->group(function(){
    Route::any('webhook-notification','flutterWebhookNotification')->name('flutterwave.webhook');
});
//Monnify
Route::controller(AddMoneyController::class)->prefix('deposit')->name('deposit.')->group(function(){
    Route::any('monnify-notification','monnifyWebhookNotification')->name('monnify.webhook');
});
